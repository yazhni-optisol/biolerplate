...
describe(`${AUTH_ENDPOINT}`, () => {
  ...
  const RESET_PASSWORD_OTP = `${AUTH_ENDPOINT}/reset_password_otp`;
  const VERIFY_BY_EMAIL = `${AUTH_ENDPOINT}/verify_email`;
  const VERIFY_BY_OTP = `${AUTH_ENDPOINT}/verify_otp`;
  ...
  describe(`POST ${RESET_PASSWORD_OTP}`, () => {
    beforeAll(async () => {
      await User.sync({ force: true });
      testUser = await User.create(UserFactory.generate({ isVerified: true }));
      await helper.apiServer.post(FORGOT_PASSWORD_ENDPOINT).send({
    ...
    describe('Error with status 400 & 404', () => {
      it('should return an error if email is not exist on db', async () => {
        const res = await helper.apiServer.post(RESET_PASSWORD_OTP).send({
          email: 'test@test.com',
          otpCode: '121212',
          password: 'passpass',
        });
      ...
      it('should return the success message', async () => {
        ...
        expect(status).toEqual(400);
        expect(body).toHaveProperty('message');
        expect(body.message.details[0].email).toEqual('"email" is required');
        expect(body.message.details[0].otpCode).toEqual('"otpCode" is required');
      });
    });
    describe('Reset Password with status 200', () => {
      it('should return the success message', async () => {
  ...
  describe(`POST ${VERIFY_BY_OTP}`, () => {
    ...
    describe('Error with status 400 & 404', () => {
      it('should return an error if otp is invalid', async () => {
        const res = await helper.apiServer.post(VERIFY_BY_OTP).send({
          otpCode: '123123',
          email: testUser.email,
        });
        const { status, body } = res;
      ...
      it('should return an error if otp/email is not available', async () => {
        const res = await helper.apiServer.post(VERIFY_BY_OTP).send({
          otpCode: '',
          email: testUser.email,
        });
        ...
        expect(status).toEqual(400);
        expect(body).toHaveProperty('message');
        expect(body.message.message).toEqual('Validation Failed');
        expect(body.message.name).toEqual('ValidationError');
        expect(body.message.details[0].otpCode).toEqual('"otpCode" is not allowed to be empty');
      });
    });
    describe('Verified Successfully with 200', () => {
      it('should return the success message', async () => {
      ...
      it('should return the already verified message', async () => {
        const res = await helper.apiServer.post(VERIFY_BY_OTP).send({
          otpCode: testUser.otpCode,
          email: testUser.email,
        });
        ...
        expect(status).toEqual(200);
        expect(body).toHaveProperty('message');
        expect(body.message).toEqual('User already verified.');
      });
    });
  });
});
